...
4. **Используя консольную команду получить ip адрес устройства, на котором вы работаете, от DHCP сервера.**
	...
	2. **DHCPDISCOVER(поиск):** клиент отправляет широковещательный запрос для поиска DHCP-сервера, чтобы получить IP-адрес.
	3. **DHCPOFFER (предложение):** сервер отвечает, предлагая параметры конфигурации.
	4. **DHCPREQUEST(запрос):** клиент подтверждает выбор полученного предложения.
	5. **DHCPACK(подтверждение):** сервер подтверждает выделение параметров, после чего клиент может использовать IP-адрес.
...
2. **Вывести размер папок `/home`, `/var`, `/var/log` (в байтах, в человекочитаемом виде)**
	...
	1. Запускаем команду `du -h`(выводит общий размер папок в байтах)
		...
		`/var`
		![part 12](img/part12.2.png)
		Скриншот 49. Вывод команды `du /var`

		![part 12](img/part12.3.png)
...
2. **Вывести размер папок `/home, /var, /var/log`.**
	...
	1. `ncdu /home, /var, /var/log`
		...
		![part 13](img/part13.2.png)
		Скриншот 55. Вывод размера папки /var

		![part 13](img/part13.3.png)
